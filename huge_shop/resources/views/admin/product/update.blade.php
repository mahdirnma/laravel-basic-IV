    ...
    <div class="w-full h-[88%] bg-gray-200 flex items-center justify-center">
        <div class="w-[90%] h-5/6 bg-white rounded-xl pt-3 flex flex-col items-center">
            ...
            <div class="flex w-full h-4/5">
                <form action="{{route('product.update',compact('product'))}}" method="post" class="w-full h-full flex flex-row-reverse">
                    @csrf
                    @method('put')
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <textarea name="description" id="description" cols="25" rows="5" class="rounded outline-0 p-2 border border-gray-400">
                                {{$product->description}}
                            </textarea>
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <input type="number" name="price" id="price" min="1000" step="1000" value="{{$product->price}}" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @if($errors->has('price'))
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20">
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <select name="entity" id="entity" class="w-2/5 h-8 border border-gray-400 rounded outline-0 pl-3">
                                <option value="1" {{$product->entity==1?'selected':''}}>yes</option>
                                <option value="0" {{$product->entity==0?'selected':''}}>no</option>
                            </select>
