    ...
    <div class="w-full h-[88%] bg-gray-200 flex items-center justify-center">
        <div class="w-[90%] h-5/6 bg-white rounded-xl pt-3 flex flex-col items-center">
            <div class="w-[90%] h-1/5 flex justify-end items-center border-b">
                <h2 class="text-xl">ویرایش بیمار</h2>
            </div>
            <div class="flex w-full h-4/5">
                <form action="{{route('patient.edit',compact('patient'))}}" method="post" class="w-full h-full flex flex-row-reverse">
                    @csrf
                    @method('put')
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            <label for="name" class="font-semibold ml-5">: نام بیمار</label>
                            <input type="text" name="name" id="name" value="{{$patient->name}}" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @if($errors->has('name'))
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <input type="number" name="age" id="age" value="{{$patient->age}}" class="w-2/5 h-8 border border-gray-400 rounded outline-0 p-2">
                            @if($errors->has('age'))
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20">
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <select name="hospital" id="hospital" class="w-2/5 h-8 border border-gray-400 rounded outline-0 pl-3">
                                @foreach($hospitals as $hospital)
                                    <option value="{{$hospital->id}}" {{$patient->hospital_id==$hospital->id?'selected':''}}>{{$hospital->title}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <textarea name="information" id="information" class="w-2/5 border border-gray-400 rounded outline-0 p-2" cols="10" rows="5">
