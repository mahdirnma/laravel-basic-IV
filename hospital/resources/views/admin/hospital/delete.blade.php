...
@section('content')
    <div class="w-full h-[88%] bg-gray-200 flex flex-col items-center justify-center">
        <h2 class="text-3xl font-bold">آیا از حذف این بیمارستان اطمینان دارید؟</h2>
        <p class="mt-10 text-3xl font-bold">{{$hospital->title}}</p>
        <form action="{{route('hospital.destroy',compact('hospital'))}}" method="post">
            @csrf
            @method('delete')
